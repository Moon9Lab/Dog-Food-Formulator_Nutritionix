...
#     table_data = {
#         "": ["Serving size", "Energy", "Caloric Density"],
#         "Value": [f"{total_weight:.2f} g", f"{total_calories:.2f} Kcal", f"{caloric_density:.2f} kcal/kg"]
#     }
...
def display_recipe_summary(response):
    ...
    total_weight = 0
    total_calories = 0
    total_water = 0  # Initialize total water content
    for food in response.get("foods", []):
        food_name = food.get("food_name", "Unknown")
        serving_qty = food.get("serving_weight_grams", 0)
        serving_unit = food.get("serving_unit", "g")
